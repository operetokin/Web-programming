...
window.onload = function () {
    ...
    var myVideoString = "<figure>";
    if (video.controls == true) {
        myVideoString += "<video width=" + video.width + " height=" + video.height + " controls>";
        for (var i = 0; i < video.source.length; i++) {
            myVideoString += "<source src=" + video.source[i].src + " type= " + video.source[i].type + " /> ";
        }
        myVideoString += "</video>";
    }
    myVideoString += "</figure>";
   videoContainer.innerHTML = myVideoString;
};